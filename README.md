...
> [!NOTE]
> **Written by Claude.** Every line of code and documentation in this
...
<p align="center"><em>The three documents as an unconfigured build produces
them — every name on them a placeholder.</em></p>
...
Every slot is filled in all the same. The shipped configuration carries
stand-ins — 五段 for the grade, ジョン・ドウ for the recipient, today's date,
山田太郎 for the signatory, 柔能制剛 for the precept — so that a first run
shows a finished sheet rather than a mostly empty one, and you can see what
the layout does before configuring anything. Every one of them is meant to be
replaced or cleared.
...
/ SciPy. Files land in `build/` as `<award>-<variant>.<ext>` — with a
`-sample` tag while the shipped stand-ins are in place, so a first build is
plainly a set of specimens.
...
crest_ring = "〇〇道場"      # four characters on the crest's lower arc

[signer]
title = "師範"               # the office that grants the award
name = "山田太郎"            # the signatory; katakana for a Western name

[motto]
text = "柔能制剛"            # an optional school precept, in its own column

[sample]                     # stand-ins for the hand-brushed slots —
name = "ジョン・ドウ"        #   delete this section for real certificates
rank = "5"
date = "today"
suffix = "sample"            # so a first build is obviously a specimen set
...
| `-n`, `--name KATAKANA` | print the recipient's name instead of leaving it blank; `''` clears the config's stand-in |
| `-r`, `--rank GRADE` | print the grade: `2` or `nidan` → 二段, `1` → 初段, or literal text; `''` clears |
| `-d`, `--date YYYY-MM-DD` | print the award date in kanji; `today` also works; `''` clears |
...
Which happens is `--rank`, `--name` and `--date`, each falling back to the
`[sample]` section of the configuration when you leave the flag off. The
shipped example fills all three so that a first build is a finished sheet;
**delete the section from your own `dojo.toml`** and the slots go back to
being blank, which is what a dōjō printing onto real stock wants. To clear one
for a single run, pass it empty:

```
python3 src/build_diploma.py -n '' -r '' -d ''   # the three slots left blank
```
...
— naming a recipient or a grade always takes that derived tag, in preference
to the `[sample]` one. Long names shrink to fit.

A grade from `[sample]` is printed only on awards with `grade_column = true`:
五段 belongs on the dan certificate and not on a licence whose heading has
already named the award. An explicit `--rank` still goes wherever you point
it, which is what makes `--rank 師範` work.
...
A first build now looks like a finished certificate rather than an empty one.
It is not one: 〇〇流柔術 grants nothing, ジョン・ドウ is nobody, and the files
say `-sample` on them. The point of filling the slots is to show the layout,
not to hand anyone a document.
